...
document.addEventListener('DOMContentLoaded',function() {
    inputCerca.onchange=evidenziaCercati;
},false);
...
function caricaFiles(user){
    ...
    .then(jsonFiles => {
        ...
        jsonFiles.forEach(file => {
            ...
            a.setAttribute("title", `${file.filename}`);
...
//funzione Cerca
function evidenziaCercati(){

    console.log(aCounter);
    for(j = 1; j < 10*aCounter; j += +4){
        aArray = document.getElementsByTagName('a')[j];
        nomeFile = aArray.getAttribute("title");
        aArray.classList.remove("evidenziato");
        
        if(nomeFile.includes(inputCerca.value)){
            aArray.classList.add("evidenziato");
